...
if(isset($_GET))
	var_dump($_GET); else echo "aucune réponse";

$retest = $_SESSION['test'];

echo "test session : ".$retest;
...
if(isset($_GET['code'])){
	...
	$user_mail = $infos->email;
	 // affiche les données sur vérification :
	 echo "<pre>";
	 echo "userinfo->get() : \n";
	 print_r($infos);
	 print_r($user_mail);
...
// à ce stade on a le mail du gars dans $user_mail, maintenant il faut vérifier qu'il est dans la bdd du site,
// mettre les variables de session à jour en fonction, et rediriger vers une page quelconque du site
